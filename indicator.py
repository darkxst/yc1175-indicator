...
class Frame:
    g_seq = 0
    def __init__(self, seq:bytes(1)=None, msg:bytearray=None):
        if msg:
            self.unpack(msg)
        else:
            self.cmd = ZERO
    ...
    def build(self, mtype:bytes(1), cmd:bytes(1), data:bytearray=b''):
        self.mtype = mtype
        self.cmd = cmd
        self.data = data

    def led(self, idx:int, effect:int, rgb:tuple):
        """Build packet to control led, can be called multiple times to add leds to packet."""
...
class ycProtocol:
    ...
    async def read_serial(self, serial:asyncio.StreamReader):
        while True:
    ...
    def log_frame(self, frame:Frame, title:str=None):
        if not title:
    ...
    async def write_serial(self, serial:asyncio.StreamWriter, frame:Frame):
        packet = frame.pack()
        if packet:
            serial.write(packet)
            await serial.drain()
...
class HassAPI:
    ...
    def light_on(self, idx:int, effect:int=1, rgb:tuple=(0,0,255)):
        led_frame = Frame()
        led_frame.led(idx, effect, rgb)
        self.comm.send_frame(led_frame)

    def light_off(self, idx:int):
        rgb=(0,0,0)
        led_frame = Frame()
